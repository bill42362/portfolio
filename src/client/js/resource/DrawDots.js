...
const DrawDots = function ({ context } = {}) {
  if (!context) {
...
DrawDots.prototype.dockBuffer = function ({ key, buffer }) {
...
DrawDots.prototype.updateBuffer = function ({ key, attribute }) {
  if (!['aPosition', 'aColor'].includes(key)) {
    throw new Error('invalid buffer name');
  }
...
DrawDots.prototype.draw = function ({
  positionAttribute,
  colorAttribute,
  targetFrameBuffer,
}) {
  ...
  context.drawArrays(context.POINTS, 0, this.buffer.aPosition.count);
};
